...
export default function InputField({ prompt, setPrompt, getResponse, isLoading }) {
    ...
    useEffect(() => {
        if (!isLoading) {
            inputRef.current?.focus(); // Focus the input field after response is received
        }
    }, [isLoading]);

    return (
        <div className="flex p-1 gap-2 lg:w-full rounded-2xl bg-white-300 dark:bg-gray-400/30 border-[0.5px] border-slate-100 focus:border-black dark:border-gray-500 items-center">
            <input
                ref={inputRef}
            ...
            {prompt && (
                <button
                    ...
                    disabled={isLoading}
                >
                    {isLoading ? <LoadingIcon /> : <SendIcon className="w-4 h-4" />}
                </button>
            )}
        </div>
    );
